...
// Créer une version du store qui fonctionne côté serveur
const createBookingStore = () => {
  return create<BookingStore>()(
    persist(
      (set, get) => ({
        bookings: [],
        selectedRoom: "salle1",
        ...
        isRoomAvailable: (roomId, date, startTime, endTime, excludeBookingId) => {
          ...
          const hasConflict = state.bookings.some((booking) => {
            ...
            if (
              excludeBookingId &&
              (booking.id === excludeBookingId || booking.id.startsWith(`${excludeBookingId}-`))
            ) {
              return false
            }
      ...
      }),
      {
        ...
        storage: createJSONStorage(() => {
          ...
          // Retourner un storage vide pour le côté serveur
          return {
            getItem: () => null,
            setItem: () => {},
            removeItem: () => {},
